...
static GtkWidget* _lib_import_get_extra_widget(dt_lib_import_metadata_t *data, gboolean import_folder)
{
  ...
  if(import_folder == TRUE)
  {
    ...
    use_solitary_jpeg = gtk_check_button_new_with_label (_(" .. ignore only when other files exist with same name"));
    g_object_set(use_solitary_jpeg, "tooltip-text", _("import jpegs if they are a single file without raw or similar. This can be useful when importing a mixed collection of pictures."), NULL);
...
static void _lib_import_evaluate_extra_widget(dt_lib_import_metadata_t *data, gboolean import_folder)
{
  if(import_folder == TRUE)
  {
    ...
    dt_conf_set_bool("ui_last/import_use_solitary_jpegs", gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON (data->use_solitary_jpeg)));
  }
